...
export async function registerRoutes(
  ...
  // Same-origin proxy for remote PDFs so the in-page PDF.js reader can fetch them
  // (cross-origin hosts like Firebase Storage often block direct browser fetches → CORS).
...
4. Respond in the SAME language as the question.`;
  ...
  const summaryPrompt = `You are an expert note-taker and explainer. Produce a clear, readable summary of the lecture so a student can FULLY understand the material just by reading it — WITHOUT watching the original video.

  Your response MUST be a JSON object with this exact structure:
  {
    "mainTitle": "Short, clear title of the lecture topic",
    "subTitle": "One sentence describing what this lecture is about.",
    "takeawaySummary": "A 2-4 sentence plain-language overview (TL;DR) of the whole lecture and why it matters.",
    "sections": [
      { "heading": "Short heading for this part of the lecture", "content": "2-4 clear sentences explaining this part in plain, simple language — the actual ideas, examples, and reasoning. Someone reading this alone should understand it without the video." }
    ],
    "definitions": [
       { "term": "Technical Term", "definition": "Clear, simple definition." }
    ],
    "takeawayPoints": ["Concise, high-value takeaway 1", "takeaway 2", "takeaway 3"],
    "studyTips": ["A short, actionable tip to study or remember this material (e.g. a mnemonic, an analogy, what to focus on)."],
    "selfCheck": ["A question the student can ask themselves to test their understanding of the lecture."]
  }

  CRITICAL RULES:
  1. "sections" is the MOST important field: walk through the WHOLE lecture in logical/chronological order (typically 4-8 sections). Each section must explain the real content clearly and simply, with enough detail that reading replaces watching. NO vague filler like "this is important".
  2. Write in clear, simple, readable language — short sentences, plain words, explain any jargon.
  3. "takeawayPoints": 4-6 concise, high-value points a student should remember.
  4. "studyTips": 2-4 genuinely useful tips (mnemonics, analogies, common mistakes to avoid, what to focus on for exams).
  5. "selfCheck": 2-4 short questions (no answers) the student can use to test whether they understood the material.
  6. "definitions": ONLY genuinely technical terms a student might not know (0-8). Use an empty array if none.
  7. Preserve all mathematical formulas in LaTeX format ($...$). Bold key terms with **markdown**.
  8. Respond in the SAME language as the input lecture.
  ...
  app.post("/api/audio/transcribe", upload.single("audio"), async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (fileExt === ".pdf") {
        ...
        try {
          // Legacy binary .ppt is NOT supported by OfficeParser (and it leaks an
          // unhandled rejection that crashes the server) — skip it and let Gemini transcribe.
          if (fileExt === ".ppt") {
            console.log("[API] Legacy .ppt detected — skipping local parser, will use Gemini transcription.");
            throw new Error("Legacy .ppt not supported by local parser");
          }
...
`;
  ...
  app.post("/api/ai/category", async (req: Request, res: Response) => {
    try {
      ...
      const engineeringSignals = /(arduino|esp32|esp8266|raspberry\s*pi|microcontroller|microprocessor|micro-?processor|breadboard|\bpcb\b|\bfpga\b|verilog|vhdl|resistor|capacitor|transistor|\bled\b|\bgpio\b|\bpwm\b|voltage|\bcircuit|soldering|\bservo\b|\brelay\b|\bohm|kirchhoff|op-?amp|logic\s*gates?|flip[\s-]?flop|\balu\b|\brisc\b|\bcisc\b|opcode|multiplexer|decoder|encoder|truth\s*table|\u0627\u0631\u062F\u0648\u064A\u0646\u0648|\u0623\u0631\u062F\u0648\u064A\u0646\u0648|\u062F\u0627\u0626\u0631[\u0629\u0647]|\u0645\u0642\u0627\u0648\u0645[\u0629\u0647]|\u0645\u0643\u062B\u0641|\u062A\u0631\u0627\u0646\u0632\u0633\u062A\u0648\u0631|\b\u062C\u0647\u062F|\u0645\u062A\u062D\u0643\u0645|\u0644\u0648\u062D\u0629 \u0627\u0644\u062A\u062C\u0627\u0631\u0628|\u0644\u062D\u0627\u0645|\u0628\u0631\u0648\u0633\u064A\u0633\u0648\u0631|\u0628\u0631\u0648\u0633\u064A\u0633\u0631|\u0642\u0644\u0627\u0628|\u0641\u0644\u0641\u0644\u0648\u0628|\u0641\u0644\u064A\u0641\u0644\u0648\u0628|\u0644\u0648\u062C\u0643|\u0631\u064A\u0633\u0643|\u0633\u064A\u0633\u0643|\u0628\u0648\u0627\u0628\u0627\u062A\s*\u0645\u0646\u0637\u0642\u064A[\u0629\u0647])/i;
...
3. "code": code sketches (Arduino/C++/Python). For each snippet provide:
   - "title" and "language" (one of: cpp, arduino, python, c, javascript).
   - "code": the FULL, COMPLETE, properly-indented program with REAL line breaks — NEVER a single line and NEVER a one-liner. Reconstruct the entire working program even if the video only shows it in pieces or partially: include the needed #includes/imports, pin/variable definitions, setup()/loop() (or main/functions), and brief inline comments. Aim for realistic, compilable code (typically 10-40+ lines).
   - "explanation": a clear paragraph describing what the program does and how it works overall.
   - "concepts": an array of 3-6 key functions/APIs/concepts used (e.g. "pinMode", "digitalWrite", "PWM", "for loop", "millis()").
   - "breakdown": an ordered array of { "part": "<one line or short block copied from the code>", "note": "<plain-language explanation of what it does>" } covering the important parts of the program (5-12 items).
...
## CODE RULES:
- "code" must be valid plain source code with proper indentation and MULTIPLE lines. JSON-escape newlines (\\n) and quotes (\\") inside the "code" string (every line break in the program becomes a \\n in the JSON string). Do NOT collapse the program onto one line. Do NOT wrap it in markdown fences.
- The "part" values inside "breakdown" must be exact fragments taken from "code".
...
Return ONLY valid JSON (no markdown):
{
  ...
  "code": [ { "id": 1, "title": "", "language": "arduino", "code": "// Blink an LED on pin 13\\nvoid setup() {\\n  pinMode(13, OUTPUT);\\n}\\n\\nvoid loop() {\\n  digitalWrite(13, HIGH);\\n  delay(1000);\\n  digitalWrite(13, LOW);\\n  delay(1000);\\n}", "explanation": "Turns an LED on and off every second.", "concepts": ["pinMode", "digitalWrite", "delay"], "breakdown": [ { "part": "pinMode(13, OUTPUT);", "note": "Sets pin 13 as an output." }, { "part": "digitalWrite(13, HIGH);", "note": "Turns the LED on." } ] } ],
...
${transcript.substring(0, 30000)}`;
  ...
  app.post("/api/ai/agent-chat", async (req: Request, res: Response) => {
    try {
      const { transcript, message, history, mode = "api", image, relatedLectures, selection, videoMoment } = req.body as {
        transcript: string;
        message: string;
      ...
      const genAI = new GoogleGenerativeAI(geminiApiKey);
      // Detect language from the USER's message first (site default is English, but if the
      // student writes in Arabic we must reply in Arabic). Fall back to the transcript only
      // when there is no typed text (e.g. image-only / highlighted-selection prompts).
      const userText = (message || "").trim();
      const hasArabic = /[\u0600-\u06FF]/.test(userText);
      const hasLatin = /[A-Za-z]/.test(userText);
      const isArabic = hasArabic
        ? true
        : hasLatin
          ? false
          : /[\u0600-\u06FF]/.test(transcript.substring(0, 200));
      ...
      // --- Grounding blocks: highlighted PDF selection / paused video moment ---
      let focusBlock = "";
      if (selection?.text && selection.text.trim()) {
        const pageRef = selection.page ? (isArabic ? ` (صفحة ${selection.page})` : ` (page ${selection.page})`) : "";
        focusBlock += isArabic
          ? `\n\n--- النص المحدد من قبل الطالب${pageRef} ---\nالطالب قام بتحديد هذا المقطع تحديداً ويسأل عنه. ركّز إجابتك على شرح هذا الجزء بالذات:\n"""\n${selection.text.substring(0, 4000)}\n"""\n---`
          : `\n\n--- Text the student highlighted${pageRef} ---\nThe student selected this exact passage and is asking about it. Anchor your answer on explaining THIS specific part:\n"""\n${selection.text.substring(0, 4000)}\n"""\n---`;
      }
      if (videoMoment?.time) {
        focusBlock += isArabic
          ? `\n\n--- لحظة الفيديو (${videoMoment.time}) ---\nأوقف الطالب الفيديو عند ${videoMoment.time} ويسأل عما يُشرح في هذه اللحظة.${videoMoment.nearbyText ? `\nأقرب نص من التفريغ لهذه اللحظة:\n"""\n${videoMoment.nearbyText.substring(0, 3000)}\n"""` : ""}\nاربط إجابتك بهذه اللحظة من المحاضرة.\n---`
          : `\n\n--- Video moment (${videoMoment.time}) ---\nThe student paused the video at ${videoMoment.time} and is asking about what's being explained at that point.${videoMoment.nearbyText ? `\nNearest transcript text to this moment:\n"""\n${videoMoment.nearbyText.substring(0, 3000)}\n"""` : ""}\nGround your answer in this moment of the lecture.\n---`;
      }

      const systemPrompt = isArabic
        ? `أنت "AI Agent"، المساعد الأكاديمي الذكي المدمج في LectureMate. تهدف إلى مساعدة الطلاب على فهم المحاضرة بعمق.

نص المحاضرة الحالية:
${transcript.substring(0, 28000)}
${relatedLecturesBlock}${focusBlock}
...
6. نسق إجابتك بطريقة احترافية وفاخرة.`
        : `You are "AI Agent", the smart academic assistant integrated into LectureMate. Your purpose is to help students deeply understand the lecture material.
...
6. Keep your formatting highly professional.`;
      ...
      const langRule = isArabic
        ? "\n\nمهم جداً: أجب دائماً باللغة العربية الفصحى فقط، بفقرات منظمة وواضحة. لا تخلط الإنجليزية إلا للمصطلحات التقنية الضرورية."
        : "\n\nIMPORTANT: Always respond in clear, well-structured English only, matching the language the student used.";

      const promptParts: any[] = [{ text: systemPrompt + langRule }];
