...
const categoryKeywords: Record<LectureCategory, string[]> = {
  ...
  technology: [
    ...
    "python", "javascript", "typescript", "c++", "java", "html", "css", "backend", "frontend",
    "برمجة", "حاسوب", "ذكاء اصطناعي", "تعلم آلة", "تطوير", "تقنية", "روبوت", "أتمتة", "كود"
  ],
  engineering: [
    "engineering", "arduino", "circuit", "electric", "electrical", "electronics", "voltage",
    "current", "resistor", "capacitor", "inductor", "transistor", "diode", "microcontroller",
    "breadboard", "pcb", "sensor", "actuator", "motor", "servo", "relay", "embedded",
    "raspberry pi", "esp32", "esp8266", "gpio", "pwm", "ohm", "kirchhoff", "signal",
    "mechanical", "civil", "cad", "soldering", "logic gate", "amplifier", "power supply",
    "هندسة", "مهندس", "دائرة", "كهرباء", "كهربائي", "إلكترونيات", "جهد", "تيار", "مقاومة",
    "مكثف", "ترانزستور", "متحكم", "حساس", "محرك", "اردوينو", "اوم"
  ],
  mathematics: [
    "math", "mathematics", "calculus", "algebra", "geometry", "statistics",
    "equation", "formula", "theorem", "proof", "derivative", "integral",
    "arithmetic", "trigonometry", "matrix", "vector", "probability",
    "رياضيات", "حساب", "جبر", "هندسة", "إحصاء", "معادلة", "تفاضل", "تكامل"
  ],
  medicine: [
...
function classifyLectureByKeywords(lecture: Partial<Lecture>): LectureCategory {
  ...
  for (const [category, keywords] of Object.entries(categoryKeywords)) {
    ...
    for (const keyword of keywords) {
      // Escape regex metacharacters so keywords like "c++" don't break the RegExp.
      const escaped = keyword.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      // Use word boundaries only next to word characters (\b before "+" never matches).
      const startB = /^\w/.test(keyword) ? "\\b" : "";
      const endB = /\w$/.test(keyword) ? "\\b" : "";
...
export function getCategoryInfo(category: LectureCategory, language: "ar" | "en" = "en") {
  const categories: Record<LectureCategory, { en: string; ar: string; icon: string }> = {
    ...
    engineering: { en: "Engineering", ar: "هندسة", icon: "⚙️" },
