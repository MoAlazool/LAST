...
export default function LectureView() {
  ...
  // Video / audio sources don't yield extracted imagery — only documents do.
  const isMediaSource =
    lecture?.sourceType === "youtube" ||
    lecture?.sourceType === "video" ||
    lecture?.sourceType === "audio" ||
    /(youtube\.com|youtu\.be)/.test(sourceHint) ||
    /\.(mp4|webm|mov|avi|mkv|mp3|wav|ogg|m4a)\b/.test(sourceHint);
  ...
  const isTaskComplete = (task: string) => {
    ...
    // Once the lecture has finished processing, every module counts as ready.
  ...
  const tasksInfo = [
    ...
    { key: 'chat',       icon: 'smart_toy',    label: language === "ar" ? 'الوكيل الذكي' : 'AI Agent', doneMsg: language === "ar" ? 'جاهز للإجابة على استفساراتك' : 'Ready to answer all your questions', pendingMsg: language === "ar" ? 'تهيئة الوكيل الذكي...' : 'Initializing AI Agent...', tab: 'chat' },
  ...
  const visibleTasks = tasksInfo.filter(task => {
    ...
    if (task.key === 'images') {
      ...
      // While processing, only show Imagery for document sources (PDF/PPTX), never for video/audio.
  ...
  const isPdfSource = /\.pdf\b/.test(sourceHint) || lecture.sourceType === "pdf";
  const typeMeta = isMediaSource
    ? { label: isRTL ? "فيديو/صوت" : "Media", icon: "movie" }
    : isPresentation
      ? { label: isRTL ? "عرض تقديمي" : "Slides", icon: "slideshow" }
      : isPdfSource
        ? { label: "PDF", icon: "picture_as_pdf" }
        : { label: isRTL ? "مستند" : "Document", icon: "description" };
  const wordCount = lecture.transcript ? lecture.transcript.trim().split(/\s+/).filter(Boolean).length : 0;
  const imageCount = lecture.extractedImages?.length || 0;
  ...
  if (!forceShowContent) {
    return (
      <AppLayout
        ...
        lectureNav={{
          items: visibleTasks.map(task => ({ key: task.key, tab: task.tab, label: task.label, icon: task.icon })),
          activeTab,
          onSelect: (tab) => { setActiveTab(tab); setForceShowContent(true); },
        }}
      >
          ...
          <div className="mb-10 max-w-[1400px] mx-auto" dir={isRTL ? "rtl" : "ltr"}>
            <div className="bg-surface-container-lowest rounded-[2.5rem] p-6 md:p-9 border border-outline-variant/30 shadow-[0_20px_60px_rgba(0,0,0,0.03)]">
              <div className={cn("flex flex-col lg:flex-row lg:items-center justify-between gap-7")}>
                ...
                {/* Reprocess control */}
                <div className={cn("shrink-0 lg:w-[300px] bg-surface rounded-[1.75rem] p-5 border border-outline-variant/30", isRTL ? "text-right" : "text-left")}>
                  <p className="text-[11px] font-black uppercase tracking-wider text-on-surface-variant mb-2.5">
                    {isRTL ? "إعادة المعالجة بمحرّك" : "Reprocess with engine"}
                  </p>
                  <div className="flex items-center gap-1 bg-surface-container-low rounded-2xl p-1 mb-3">
                    {(["api", "gpu"] as const).map((m) => (
                      <button
                        key={m}
                  ...
                  {(lecture.status === "processing" || isReprocessing) && (
                    <button
                  ...
                  {visibleTasks.map((task, idx) => {
                    ...
                    return (
                      <div key={task.key} className={cn(
                        "rounded-[2.5rem] p-10 flex flex-col justify-between hover:shadow-[0_40px_80px_rgba(0,0,0,0.1)] transition-all duration-700 group relative overflow-hidden min-h-[340px] border",
                        done 
  ...
  return (
    <AppLayout
      ...
      lectureNav={{
        ...
        onBack: () => setForceShowContent(false),
      }}
    >
        ...
        >
          <Tabs defaultValue="summary" value={activeTab} onValueChange={setActiveTab} className="w-full">
            <div className="min-h-[500px]">
              ...
              <TabsContent value="chat" className="mt-0 h-[85vh] min-h-[640px] border-none p-0 focus-visible:ring-0 overflow-hidden rounded-2xl shadow-sm">
                <AgentChatView
                  transcript={lecture.transcript || ""}
                  title={lecture.title}
                  lectureId={lecture.id}
