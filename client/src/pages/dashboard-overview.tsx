...
import { BookOpen, TrendingUp, ArrowRight, FileText, FileVideo, Presentation, CheckCircle2, Calendar, Flame, Award, Search, Loader2, Play, BarChart3, Layers, Plus } from "lucide-react";
...
function LectureCard({ lecture, label }: { lecture: Lecture; label: { ready: string; processing: string } }) {
  ...
  const placeholderBg =
    fileType === "pdf"
      ...
      : fileType === "pptx"
      ? "from-primary/5 to-primary/10 text-primary"
  ...
  return (
    <Link
      ...
      className="group block bg-white rounded-[20px] overflow-hidden border border-slate-200/70 hover:border-primary/40 shadow-sm hover:shadow-[0_14px_38px_rgba(240,90,34,0.14)] hover:-translate-y-1 transition-all duration-300 no-underline"
    >
        ...
        {hasThumb ? (
          <>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-12 h-12 rounded-full bg-white/15 backdrop-blur-md border border-white/30 flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:bg-primary transition-all duration-300">
      ...
      <div className="p-4">
        <p className="font-bold text-slate-900 text-sm leading-snug line-clamp-2 mb-2 group-hover:text-primary transition-colors">
          {lecture.title}
        </p>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-slate-200/80 pb-7">
              ...
              {stats.total > 0 && (
                <div className="mt-5 max-w-xs">
                  <div className={cn("flex items-center justify-between text-[11px] font-bold mb-1.5")}>
                    ...
                    <span className="text-primary tabular-nums">{stats.completion}%</span>
                  </div>
                  <div className="h-2 rounded-full bg-slate-200/70 overflow-hidden">
                    <div
                      className="h-full rounded-full bg-primary transition-[width] duration-[1000ms] ease-out"
            ...
            <div className={cn("flex items-center gap-3 shrink-0")}>
              ...
              <Link
                href="/"
                className={cn(
                  "no-underline inline-flex items-center gap-2 bg-slate-900 hover:bg-primary text-white font-bold text-sm px-5 py-3 rounded-full shadow-lg shadow-slate-900/10 hover:shadow-primary/25 transition-all active:scale-95"
                )}
              >
                <Plus size={16} />
                {t.newAnalysis}
                    ...
                    <div className="w-full flex items-end justify-center h-full">
                      <div
                        className={cn(
                          ...
                          d.count > 0 ? "bg-primary group-hover:bg-primary" : "bg-slate-100"
                        )}
                        style={{
                  ...
                  {categories.map((c, i) => (
                    <div key={i}>
                      ...
                      <div className="h-2 rounded-full bg-slate-100 overflow-hidden">
                        <div
                          className="h-full rounded-full bg-gradient-to-r from-primary to-[#f5793f] transition-[width] duration-700 ease-out"
          ...
          <div>
            <div className={cn("flex items-center justify-between mb-5")}>
              ...
              <Link
                href="/history"
                className={cn(
                  "no-underline group flex items-center gap-1.5 text-sm font-bold text-slate-500 hover:text-primary transition-colors"
                )}
              >
                {t.viewAll}
            ...
            {isLoading ? (
              ...
              <div className="bg-white rounded-[28px] p-16 text-center border border-slate-200/70">
                ...
                <Link
                  href="/"
                  className="no-underline inline-flex items-center gap-2 bg-primary text-white font-bold text-sm px-6 py-3 rounded-2xl hover:brightness-110 transition-all"
                >
                  <Plus size={16} /> {t.analyzeFirst}
                </Link>
              </div>
              ...
              <div className="bg-white rounded-[28px] p-12 text-center border border-slate-200/70">
                ...
                <button
                  ...
                  className="mt-4 text-primary font-bold text-sm hover:underline border-0 bg-transparent cursor-pointer"
                >
                  {t.clearSearch}
                </button>
              </div>
