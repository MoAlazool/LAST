            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
                <div className="space-y-2 min-w-0">
                    <h1 className="text-4xl lg:text-5xl font-black tracking-tight text-slate-900 flex items-center gap-3">
                        <Stethoscope className="w-9 h-9 text-primary shrink-0" />
                        {t.title}
                    </h1>
                    <p className="text-xs text-slate-400 truncate max-w-md">
                        {t.subtitle} <span className="text-primary/80 font-medium">{lectureTitle}</span>
                    </p>
                </div>
                <button
...
// Cards size to their own content (no h-full stretch) and are kept whole inside a column.
const cardClass = "flex flex-col rounded-[24px] border border-[#F1F5F9] bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] transition-all duration-500 break-inside-avoid mb-6 w-full";

// Masonry layout: cards flow into balanced columns and pack vertically, so a short
// card sits directly under the one above it instead of stretching to match a tall neighbour.
function Masonry({ children }: { children: React.ReactNode }) {
    return <div className="columns-1 lg:columns-2 gap-6 [column-fill:balance]">{children}</div>;
}
