...
import { CodeBlock, normalizeCode } from "./CodeBlock";
...
export function EngineeringLabView({ engineering, lectureTitle = "Engineering Lecture" }: EngineeringLabViewProps) {
    ...
    const handleExportPDF = async () => {
        try {
            ...
            const codeHtml = code.length ? sectionHeader(isAr ? "الأكواد" : "Code") + code.map(x => `
                <div class="card"><h3>${esc(x.title)}</h3><pre dir="ltr">${esc(normalizeCode(x.code))}</pre>${x.explanation ? `<p class="muted">${esc(x.explanation)}</p>` : ""}${(x.breakdown && x.breakdown.length) ? `<table>${x.breakdown.map(b => `<tr><td class="k" dir="ltr">${esc(b.part)}</td><td>${esc(b.note)}</td></tr>`).join("")}</table>` : ""}</div>`).join("") : "";
    ...
    return (
        <div dir={isAr ? "rtl" : "ltr"} className={`max-w-7xl mx-auto py-8 font-display animate-in fade-in duration-500 ${isAr ? "text-right" : "text-left"}`}>
            ...
            {activeSection === "code" && <CodeList items={fCode} isAr={isAr} />}
...
function CodeList({ items, isAr }: { items: CodeSnippet[]; isAr: boolean }) {
    return (
        <div className="space-y-6">
            {items.map((x, idx) => {
                const lineCount = normalizeCode(x.code).split("\n").filter(Boolean).length;
                return (
                <motion.div key={x.id || idx} {...cardMotion(idx)} className="rounded-[24px] border border-[#F1F5F9] bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                    ...
                    {/* Explanation */}
                    {x.explanation && (
                        <div className="mt-4 rounded-2xl bg-[#F8FAFC] border border-slate-100 p-4">
                            <p className="text-[11px] font-black uppercase tracking-wider text-slate-400 mb-1.5">{isAr ? "الشرح" : "Explanation"}</p>
                            <p className="text-sm text-slate-600 leading-relaxed">{x.explanation}</p>
                        </div>
                    )}

                    {/* Line-by-line breakdown */}
                    {x.breakdown && x.breakdown.length > 0 && (
                        <div className="mt-4">
